        ...
        try {
            if (current_id == -1)
                ...
                return new Query (db, "SELECT * FROM %s WHERE %s = %d".printf (
                    tables[current_step],
                    parent_columns[current_step],
                    m_node_list[current_id].id));
        ...
        if (current_id == -1) {
            ...
            switch (current_step) {
                ...
                case ChildType.ENCLOSURE:
                    try {
                        for (; !res.finished; res.next ()) {
                            (m_node_list[current_id].data as Feed).get_item (
                                    res.fetch_string (
                                        res.field_index ("item_guid")
                                    )).attachments.add (new Attachment.from_record (res));
                        }
                    } catch (SQLHeavy.Error e) {
                        error ("Failed to build feed structure: %s", e.message);
                    }
                    ...
                    try {
                        for (; !res.finished; res.next ()) {
                            (m_node_list[current_id].data as Feed).get_item (
                                res.fetch_string (
                                    res.field_index ("item_guid")
                                )).author = new Person.from_record (res);
                        }
                    } catch (SQLHeavy.Error e) {
                        error ("Failed to build feed structure: %s", e.message);
                    }
