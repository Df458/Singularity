...
// A DatabaseRequest for subscribing to new feeds
...
public class SubscribeRequest : DatabaseRequest, GLib.Object {
    ...
    private void insert_node (StringBuilder q_builder, CollectionNode n, bool first) {
        ...
        if (n.data is Feed) {
            ...
            q_builder.append_printf (" (%d, %d, %s, %s, %s, %s, %s, %s, %lld)",
                p_id,
                (int)CollectionNode.Contents.FEED,
                sql_str (f.title),
                sql_str (f.link),
                sql_str (f.site_link),
                sql_str (f.description),
                sql_str (f.rights),
                sql_str (f.generator),
                f.last_update.to_unix ());
        } else {
            FeedCollection c = n.data as FeedCollection;
            q_builder.append_printf (" (%d, %d, %s, null, null, null, null, null, null)",
                p_id,
                (int)CollectionNode.Contents.COLLECTION,
                sql_str (c.title));
        }
    }
}
