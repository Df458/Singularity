        ...
        public Query build_query(Database db)
        {
            switch(m_status) {
                case Status.ICON_INSERT:
                    try {
                        Query q = new Query(db, "INSERT OR REPLACE INTO icons (id, width, height, alpha, bits, rowstride, data) VALUES (%d, %d, %d, %d, %d, %d, :data)".printf(package.feed.id, package.feed.icon.width, package.feed.icon.height, package.feed.icon.has_alpha ? 1 : 0, package.feed.icon.bits_per_sample, package.feed.icon.rowstride));
                ...
                case Status.INSERT_PEOPLE:
                    ...
                    foreach(Item i in package.new_items) {
                        if(i.author != null) {
                            ...
                            q_builder.append_printf("(%d, %s, %s, %s, %s, %s)", package.feed.id, sql_str(i.guid), sql_str(i.author.guid), sql_str(i.author.url), sql_str(i.author.name), sql_str(i.author.email));
                            first = false;
                        }
                    }
                    try {
